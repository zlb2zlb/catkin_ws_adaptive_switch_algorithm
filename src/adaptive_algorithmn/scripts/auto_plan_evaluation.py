...
def main():
    ...
    for j in cylinder_number:
        for i in range(world_number_start,world_number_end):
            for k in range(10):
                # 设置环境变量
                os.environ['MAP_FILE'] = '/home/zlb/worlds/auto_world_maps/slim_cylinder_10/world_cylinder_' + str(j) + '_' + str(i) + '.yaml'
                os.environ['WORLD_DIR'] = '/home/zlb/worlds/auto_world_maps/slim_cylinder_10/world_cylinder_' + str(j) + '_' + str(i) + '.world'
                time.sleep(1)

                # 启动gazebo
                gazebo_launch = roslaunch.parent.ROSLaunchParent(uuid, ["/home/zlb/dev/ros_collections/catkin_ws/src/turtlebot3_simulations/turtlebot3_gazebo/launch/turtlebot3_world.launch"])
                gazebo_launch.start()
                time.sleep(20)
            
                # 启动导航rviz
                nav_launch = roslaunch.parent.ROSLaunchParent(uuid, ["/home/zlb/dev/ros_collections/catkin_ws/src/turtlebot3/turtlebot3_navigation/launch/turtlebot3_navigation.launch"])
                nav_launch.start()
                time.sleep(10)
